 ...
 * @Description: 身份证严格验证工具类
 * @date 2014-11-24 上午9:59:05
 */
public class IDCardUtil {
    ...
    private int getAgeByUserId(String arg_id) {
        int result = -1;
        ...
        String ageS = arg_id.substring(6, 10);
        if (!AppUtil.isPositiveInteger(ageS))
            return result;
        int birthyear = Integer.parseInt(ageS);
     ...
     * @return
     */
    public static int checkGender(String idNo) {
        if (!AppUtil.isEmpty(idNo) && verify(idNo)) {
            int genderFlag = idNo.length() == 18 ?
                    Integer.parseInt(idNo.substring(idNo.length() - 2, idNo.length() - 1))
                    : Integer.parseInt(idNo.substring(idNo.length() - 1, idNo.length()));
            return genderFlag % 2 == 0 ? FEMALE : MALE;
        }
        return MALE;
    }
}
